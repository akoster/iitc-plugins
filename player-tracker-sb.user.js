// ==UserScript==
// @id             iitc-plugin-player-tracker-sb@breunigs
// @name           IITC Plugin: Player tracker SB
// @category       Layer
...
window.plugin.playerTracker.setup = function() {
  ...
  plugin.playerTracker.drawnSpeedBubbles = new L.LayerGroup();
  window.addLayerGroup('Speed bubbles', plugin.playerTracker.drawnSpeedBubbles, true);
...
plugin.playerTracker.onClickListener = function(event) {
  ...
  var playerName = marker.options.title.split(',')[0];
  var playerData = window.plugin.playerTracker.findUser(playerName);
  var lastPlayerEvent = playerData.events[playerData.events.length-1];
  var secondsSinceLastPlayerEvent = (now-lastPlayerEvent.time) / 1000;
  var bubbleRadiusInMetres = 15.6464 * secondsSinceLastPlayerEvent;
  var position = marker.getLatLng();
  console.log("position: " + position + " secondsSinceLastPlayerEvent:" + secondsSinceLastPlayerEvent + " bubbleRadiusInMetres:" + bubbleRadiusInMetres);
  plugin.playerTracker.drawnSpeedBubbles.clearLayers();
  L.circle(position, bubbleRadiusInMetres, {weight:1, color:"FF0000"}).addTo(plugin.playerTracker.drawnSpeedBubbles);
